...
export abstract class SvgItem {
  ...
  public refreshAbsolutePoints(_origin: Point, previous: SvgItem | null) {
  ...
  public asStandaloneString(): string {
    return ['M', this.previousPoint.x, this.previousPoint.y, this.getType(), ...this.values].join(' ')
  }
...
class CurveTo extends SvgItem {
  ...
  public refreshAbsoluteControlPoints(_origin: Point, previousTarget: SvgItem | null) {
    if (!previousTarget)
      // eslint-disable-next-line no-throw-literal
      throw 'Invalid path'
...
class SmoothCurveTo extends SvgItem {
  ...
  public setControlLocation(_idx: number, pts: Point) {
...
class VerticalLineTo extends SvgItem {
  ...
  public scale(_kx: number, ky: number) {
    this.values[0] *= ky
  }
...
class EllipticalArcTo extends SvgItem {
  ...
  public translate(x: number, y: number, _force = false) {
    if (!this.relative) {
      this.values[5] += x
      this.values[6] += y
    }
